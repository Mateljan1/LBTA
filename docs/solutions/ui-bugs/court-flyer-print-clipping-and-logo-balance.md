...
- **Overlapping slots:** Merged cells list **`{time} — {program}`** per line (sorted by start) so Saturday LBHS shows each class time; whole-block span still uses union `slot.time` in a footer (“Adjacent courts · block …”). Concurrent cells use a **white + inset border** (not a single pastel) so mixed programs don’t read as one muddy color. Flyer/PDF single-session cells repeat **session time** under the title for scanability.
