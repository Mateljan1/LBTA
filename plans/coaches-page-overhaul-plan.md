...
- **UX**: No in-page navigation for long content; hero parallax may not respect `prefers-reduced-motion`; a legacy featured section still references `robert.webp` instead of the current canonical image naming.
...
- [x] **1.3** Fix coaches page: In the legacy featured section, change `src="/images/coaches/robert.webp"` to the canonical image path used by the active coach data and bio pages.
...
- [x] **2.3** Create `components/coaches/CoachCard.tsx`: Single coach card (image, title, name, specialization, availability, bio snippet, credentials, “View full bio” link when slug present). Used for both lead and program grid.
- [x] **2.4** Create `components/coaches/CoachingTeamSection.tsx`: Section title (“Meet the Team”), one featured lead card, then grid of program coaches using `CoachCard`. Data from `getCoaches()`.
...
- [x] **4.1** Generate JSON-LD in page (or a small `CoachesSchema` component) from `getCoachesForSchema()`. Include for each Person: `name`, `jobTitle`, `description`, `image` (absolute URL), `url` (e.g. `https://lagunabeachtennisacademy.com/coaches/peter-defrantz`). Keep ItemList wrapper.
...
| `components/coaches/CoachingTeamSection.tsx` | Create | Section + featured lead + grid |
...
| `app/coaches/page.tsx` | Modify | Compose components; use coaches-data; fix legacy image path; schema from data |
...
- [ ] Coaches page builds and renders correctly; legacy image references are removed and canonical coach image paths are used everywhere.
...
- **Image paths**: Use consistent asset names (e.g. `peter-defrantz.png`) across listing and bio.

## Risks & Mitigations

| Risk | Mitigation |
|------|------------|
| Breaking existing bio pages (andrew-mateljan, former coach removed, etc.) | Keep static bio pages; they can later import from coaches-data for copy only, or migrate to one dynamic `[slug]` route in a follow-up. |
