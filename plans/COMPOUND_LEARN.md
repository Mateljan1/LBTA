...
**Latest:** Lighthouse / performance pass (2026-03-28, commit a4e51f7): Homepage hero LCP = `next/image` priority on `hero-poster.webp`; video `preload=none`, deferred `load()`, opacity fade on `onPlaying`; reduced-motion = poster only. Root layout: removed global poster preload; GA4 + Meta Pixel `next/script` `lazyOnload`; Facebook `dns-prefetch` instead of `preconnect`. Coach Hub: `middleware.ts` → **`proxy.ts`** (`export function proxy`). Patterns: `hero-video-poster-lcp` (updated), `nextjs16-proxy-coach-hub`, `analytics-scripts-lazyOnload-lcp`. Anti-patterns: `root-layout-preload-homepage-only-lcp`, `hero-poster-triple-fetch`. Quality bars: `homepageHeroLcpPosterImage`, `thirdPartyScriptLoadStrategy`. [Learn doc: .cursor/compound/learnings/2026-03-28-lighthouse-proxy-compound-learn.md](../.cursor/compound/learnings/2026-03-28-lighthouse-proxy-compound-learn.md). Earlier: UTR Match Play standalone page (2026-03-23): lib/utr-match-play.ts as single source for season, divisions, modal payload from leagues-2026.json; LuxuryYearModal wired as primary CTA (replaces mailto); form-config duration derived via getUtrCircuitFormDuration(); AC division tags use data.division; getUtrDivisionTag normalizes en-dash; leagues schema extended with ntrpToUtr. ActiveCampaign test email fix (2026-03-20): v3 POST /campaigns/{id}/test returns 405 → use legacy campaign_send with campaignid + messageid. Agent-native architecture — Phases 6 → 3 → 2 implemented (2026-03-20): `scripts/agent-tools/` action parity, `lib/chat-copy.ts` prompt-native stub copy, `lib/agent-auth.ts` + `AGENT_SECRET`, context injection on `/api/chat` (`pathname` + `history`). [Solution doc: agent-tools-action-parity-implementation.md](../docs/solutions/architecture/agent-tools-action-parity-implementation.md). **Next:** re-score `AGENT-NATIVE-ARCHITECTURE-AUDIT.md`, then roadmap Phases 1 / 4 / 5. Earlier: full-site 8-principle audit — 2026-03-19. [Audit report](../docs/AGENT-NATIVE-ARCHITECTURE-AUDIT.md). [Audit solution doc](../docs/solutions/architecture/agent-native-architecture-audit.md). Earlier: Photos & layouts audit — 2026-03-19 (site-wide image 404s → legacy paths, hero alts, objectPosition, philosophy pillars, Facebook pixel a11y). [Documented solution: site-wide image 404s and layout consistency](../docs/solutions/ui-bugs/site-wide-image-404s-layout-consistency.md). Optional CI: `node scripts/verify-image-paths.mjs`. Visual Elevation Phase 3 & 4 (MasonryGrid/ZigzagSection/SplitSection; next/image quality=90). Phases 5–9: [visual-elevation-conversion-strip-facility-quote-pattern](../docs/solutions/implementation-patterns/visual-elevation-conversion-strip-facility-quote-pattern.md). Coaches headshots — 2026-03-18.
...
| Original | Correction |
|----------|------------|
| Global layout `<link rel="preload" as="image">` for a homepage-only LCP asset | Remove from root layout; let homepage `HomeHero` own the fetch with `next/image` `priority` so `/schedules` and other routes do not preload it. |
| Hero: same poster URL via layout preload + `<video poster>` + `next/image` | Use one mechanism: `next/image` priority layer + video without `poster` (or drop layout preload); video `preload="none"` and deferred `load()` so LCP stays on the image. |
| Coach Hub protection in `middleware.ts` on Next.js 16 | Use `proxy.ts` at root with `export function proxy` and `config.matcher`; same auth logic; verify `ƒ Proxy (Middleware)` in build output. |
...
| Footer/dark body text at text-white/50 failing 7:1 in validation | Use text-white/70 for body, tagline, and nav links on bg-brand-deep-water so contrast meets WCAG AAA (7:1). |
...
| Error page CTAs without 48px or focus ring | app/error.tsx "Try again" / "Return home" had no min-height or visible focus. | Use min-h-[48px], focus-visible:ring-2 focus-visible:ring-brand-victoria-cove focus-visible:ring-offset-2 focus-visible:ring-offset-white (light page). |
| Footer focus ring at 30% opacity on deep-water | focus-visible:ring-white/30 may fail WCAG AAA focus-indicator contrast on dark. | Use focus-visible:ring-white (solid) and focus-visible:ring-offset-brand-deep-water. |
| Gradient hero CTA with ring-offset-transparent | Focus ring can be hard to see on dark gradient. | Use focus-visible:ring-offset-black/50 so ring offset is visible. |
| Hero with fixed header and no top padding | Headline ("Tennis, as it should be taught") is hidden behind nav bar on load. | Add pt-20 md:pt-24 (or match header height) to hero section so content starts below the banner. |
| Hero headline still clipped in some browsers (e.g. Chrome) even with min-h-screen and top-20/24 | Browser/zoom differences can hide top of content. | Use extra top clearance: content area top-24 md:top-28 and pt-16 md:pt-20 so headline is never clipped across Chrome, Safari, Firefox. |
| Body or description text in footer area (e.g. NewsletterForm) on deep-water at text-white/50 | Fails WCAG 7:1 on bg-brand-deep-water. | Use text-white/70 or higher for any body/description in components rendered inside the footer (same background). NewsletterForm description fixed to text-white/70. |
| Webhook endpoint without rate limiting | External webhooks (e.g. ActiveCampaign) can be abused or cause load. | Add `rateLimit('webhook:'+ip, RATE_LIMITS.webhook)` at top of handler with try/catch; allow request on rate-limit failure so KV outage does not block. Return 401 (not 503) in production when webhook secret env is unset. |
...
| Middleware protects route (e.g. /coach-hub) with env secret but returns next() when secret unset | Hub is reachable without auth when COACH_HUB_SECRET missing. | When path is protected and secret missing, redirect to login or return 503; never allow access when unconfigured. |
...
| AC division tag lookup using data.program for UTR registrations | data.program holds program name (e.g. "UTR Match Play Series — Season 1"); division is in data.division. | Use data.division \|\| data.program for getUtrDivisionTag(); register-year passes division from modal. |
| Division labels with en-dash (UTR 2.0–4.0) failing slug lookup | toLowerCase().replace(/\s+/g, '-') leaves U+2013 en-dash; slug map uses ASCII hyphen. | Normalize: .replace(/[\u2013\u2014\u2212]/g, '-') before slug lookup in getUtrDivisionTag. |
| form-config duration hardcoded for utr-circuit | "8 Saturdays (Apr 11 - May 30)" drifted from leagues-2026.json seasonLabel. | Derive via getUtrCircuitFormDuration() from JSON; add test that duration matches season. |
| Leagues Zod schema omitted ntrpToUtr | parseLeagues strips unknown keys; ntrpToUtr used by page. | Add ntrpToUtr to leagues utr schema so parsed data includes it. |
| Program landing with mailto primary when modal + API exist | Conversion lost; no AC tags, no CRM flow. | Wire LuxuryYearModal as primary CTA; keep mailto as secondary for edge questions. |
...
| Auth contract verification script | Protected route has auth API (400 invalid body, 401 wrong password) | Add scripts/verify-*-auth.mjs; run with dev server (COACH_HUB_SECRET=secret next dev), then node script; assert 400 and 401 so validation can reach 100/100. |
| Modal focus trap Tab wrap | Modal/overlay has initial focus + Escape; WCAG expects focus to stay in dialog | In onKeyDown: Tab on last focusable → focus first; Shift+Tab on first → focus last. Query focusables from dialog ref; filter disabled. |
...
| Program landing single source from JSON | Program page (e.g. UTR Match Play) needs season, divisions, modal payload, form prefill | Create lib facade (e.g. utr-match-play.ts) over leagues-2026; export getUtrSeasonLabel, getUtrDivisionsForPage, getUtrCircuitModalData, getUtrCircuitFormDuration; page, modal, form-config all import. |
| Form prefill duration from data | form-config prePopulateData.duration for program with season dates in JSON | Derive via helper (e.g. getUtrCircuitFormDuration()) that reads seasonLabel; add test asserting duration contains year and matches JSON. |
...
| Hero cross-browser headline clearance | Hero with fixed header and bottom-pinned headline; must be fully visible on load in all major browsers | Content area: top-24 md:top-28; pt-16 md:pt-20 on content block. Avoids Chrome/Safari/Firefox and zoom clipping top of headline. |
| Plan implementation status before next | Starting work from existing plans (schedules UX, brand guide, Canva) | Review code against plan checklist; update plans/plan-implementation-status-*.md. Avoids re-implementing work already done. |
| Footer-area deep-water contrast | Any component rendered inside the footer (same bg-brand-deep-water) | Use text-white/70 or higher for body and description text (e.g. NewsletterForm). Matches footer tagline and links; avoids text-white/50 which can fail 7:1. |
...
| Hero headline fully visible on load in major browsers (Chrome, Safari, Firefox); use extra top clearance if any browser clips | Must — top-24 md:top-28, pt-16 md:pt-20 when fixed header + bottom-pinned content. |
| All body/description text on deep-water (footer and components in footer, e.g. NewsletterForm) use text-white/70 or higher | Must — WCAG 7:1 on bg-brand-deep-water. |
| Webhook endpoints must have rate limiting (webhook:ip key, try/catch allow on failure) | Must |
...
- Webhook endpoint without rate limiting (add rateLimit with webhook:ip, try/catch)
...
| 2026-03-11 | `/compound:full` A11y 100/100 follow-up | **Work:** (1) app/error.tsx — Try again / Return home: min-h-[48px], focus-visible:ring-2 ring-brand-victoria-cove ring-offset-white. (2) Footer — all interactive elements: focus-visible:ring-white/30 → focus-visible:ring-white (solid) for WCAG AAA on deep-water. (3) contact/page hero CTA — focus-visible:ring-offset-black/50 for gradient hero. **Deploy:** commit, push, vercel --prod. **Learnings:** 3 corrections (error page CTAs; footer ring solid; gradient hero ring-offset); 3 quality bars (errorPageCtaA11y, footerFocusRingSolid, gradientHeroCtaFocusOffset). |
| 2026-03-11 | `/compound:learn` (full run) | **Scope:** Hero clearance (Chrome headline clip), coaches deploy, full validation, plan status review. **Extraction:** (1) Hero headline clipped in some browsers even with min-h-screen and top-20/24 → use extra top clearance (top-24 md:top-28, pt-16 md:pt-20). (2) Before executing “next plans,” check implementation status so schedules UX / brand guide / Canva aren’t redone. **Updates:** corrections.jsonl +1 (hero cross-browser clearance); patterns.json +2 (hero-cross-browser-headline-clearance, plan-implementation-status-before-next); quality-bars.json +1 (heroHeadlineVisibleAllBrowsers); COMPOUND_LEARN.md log + standards. |
| 2026-03-11 | `/compound:full` Track B only (homepage overhaul) | **Scope:** Track B from optional-polish-and-homepage-master-plan: B.1–B.4 (layout, copy, imagery, a11y). **Work:** ExitIntentPopup comment (never show on load); HomeCTAForm id + aria-describedby on submit when error; hero tagline from data only; CTA guarantee text-white/70; Footer text-white/70 and icons 60 for WCAG 7:1. **Review:** Security PASS; A11y + Simplicity warnings fixed. **Validate:** Functional PASS; UI PASS after Footer contrast fix. **Learnings:** 1 correction (footer/dark body text-white/70 for 7:1). Pre-deploy: build + lint pass. |
| 2026-03-11 | `/compound:full` Track C (registration flow & operations) | **Scope:** optional-polish-and-homepage-master-plan Track C: one thank-you by type, ops doc, AC checklist, optional scholarship→AC, optional modal→thank-you redirect. **Work:** Thank-you page reads searchParams.type (trial/program/year/scholarship), COPY_BY_TYPE + getThankYouType with array normalization; docs/registration-flows-and-ops.md (flow table, where to look, AC confirmation checklist); scholarship API optional AC upsert + Scholarship tag when env set; LuxuryYearModal/LuxuryRegistrationModal success → "View next steps" redirect to /thank-you?type=year|program; CAMPAIGN_TAGS.scholarship: 108. **Review:** Security PASS; Simplicity approved; A11y warnings → fixed: 48px Close + Return to Homepage, focus primary on success, searchParams array normalize. **Validate:** Build + lint pass; thank-you dynamic. **Deploy:** Commit (7 files), push, vercel --prod. **Learnings:** Pattern: thank-you type whitelist + array normalize; modal success focus primary for a11y; optional AC gated by hasEnvVar (same as other routes). |
| 2026-03-11 | `/compound:full` Track A only (schedules optional polish) | **Scope:** optional-polish-and-homepage-master-plan Track A: active section in sticky nav + plan doc ticks. **Work:** SchedulesAnchorNav — IntersectionObserver, activeSectionId, aria-current, active style; isMountedRef guard for observer/matchMedia after unmount; reduceMotion state for transition; focus-visible for ring; threshold [0, 0.2, 0.5, 1]. **Review:** Security PASS; Races PASS (isMountedRef); Simplicity (threshold simplified); Pattern (focus-visible). **Validate:** Bug — ratio > 0.2 never reached with tall sections + rootMargin; fix: pick section with largest ratio (ratio > 0). **Deploy:** commit (Track A files only), push, vercel --prod. **Learnings:** 1 correction (scroll-spy fixed threshold vs tall sections → use largest ratio); 1 pattern (scroll-spy-active-section-tall-sections). |
| 2026-03-11 | `/compound:learn` after review + validate Tracks A/B/C | **Scope:** Compound review + validate for optional-polish-and-homepage-master-plan (all three tracks). **Review:** 5 agents (Security, Performance, A11y, Pattern, Simplicity) — PASS with warnings. **Fix applied:** NewsletterForm description was text-white/50 on deep-water; changed to text-white/70 for WCAG 7:1. **Learnings:** 1 correction (body/description text in footer area on deep-water → text-white/70); 1 pattern (footer-area-deep-water-contrast); 1 quality bar (deepWaterBodyTextContrast). |
| 2026-03-10 | `/compound:full` plan checkboxes + Track B verified + AC setup checklist | **Scope:** (1) Tick plan checkboxes so work is committed and tracked; (2) Track B copy/imagery verified; (3) AC automations actionable checklist. **Work:** optional-polish-and-homepage-master-plan (A/B/C steps and success criteria [x]); homepage-overhaul-plan (Phases 1–4 + Success Criteria [x]); schedules-page-ux-overhaul-plan (Lighthouse note "verify when ready"); new docs/activecampaign-setup-checklist.md (step-by-step List 4, tags 82/107/108); registration-flows-and-ops cross-ref. **Review:** Pattern/doc agent — PASS; fixed plan doc refs (activecampaign-confirmation-checklist → activecampaign-setup-checklist). **Validate:** Build + lint pass. **Deploy:** commit, push, vercel --prod. **Learnings:** Plan doc checkbox hygiene: tick only implemented items; one actionable checklist doc per external system (e.g. AC) with IDs from code; keep plan doc filenames and refs in sync. |
| 2026-03-10 | `/compound:learn` after lead + Vercel CLI | **Scope:** Lead CLI (`npm run lead` → scripts/lead.js) and Vercel CLI check (`npm run check:vercel` → vercel whoami + list). **Learnings:** 1 pattern (ops-cli-entry-points: key ops flows get named npm scripts; script prints status only, no secrets; point to docs when relevant). 1 quality bar (opsCliForKeyFlows: lead pipeline and deploy platform have CLI entry points so status is checkable from terminal). |
| 2026-03-12 | `/compound:review` AC/GHL + `/vercel-deploy` | **Scope:** Compound review (Security, Pattern, Simplicity) of AC/GHL integration; then full deploy. **Review:** Pattern agent applied external-api-error-log-safe to lib/activecampaign.ts (production = status only; dev = truncated body). Simplicity: removed unused getProgramCategory from book/route. **Deploy:** Commit 03f6dee (lead CLI, check:vercel, AC logging, book cleanup, docs), push, vercel --prod. Production: https://lbta-website.vercel.app. **Learnings:** No new corrections or patterns; existing deploy convention and error-logging pattern applied. |
| 2026-03-12 | Vercel env plan + AC vars added to production | **Scope:** Compound plan (vercel-env-and-connections-plan.md) to add ACTIVECAMPAIGN_URL, ACTIVECAMPAIGN_API_KEY (and optional GHL_WORKFLOW_ID) via MCP/CLI/API. **Work:** scripts/vercel-env-add.js (reads VERCEL_TOKEN from .env.vercel, values from process.env; POST to Vercel API); user provided AC URL and key; script added both to Vercel production + preview; redeployed. **Result:** Production now has AC configured; newsletter/trial submissions sync to ActiveCampaign. **Learnings:** 1 pattern (vercel-env-add-via-api: add sensitive Vercel env programmatically via script + API; token from gitignored file, secrets from process.env only; never commit or log values). |
| 2026-03-12 | `/compound:learn` after AC/GHL Lead Connector + smoke test | **Scope:** Lead Connector (services.leadconnectorhq.com) default, connection-check --ping, vercel-env-add/connection-check security fixes, smoke-lead.js, docs. **Corrections:** (1) Script calling external API (e.g. Vercel) must not put full response body in Error or log — reject with status only. (2) CLI catch block must log only err.message (or safe string), not full error object. **Patterns:** lead-pipeline-smoke-test (npm run smoke:lead; optional SMOKE_BASE_URL); connection-check-same-base-as-app (GHL ping uses same base/version as app). **Anti-pattern:** script-reject-with-full-response-body. **Quality bar:** leadPipelineSmokeTest (smoke test for lead pipeline without creating contacts). |
| 2026-03-12 | `/compound:learn` after fix-everything-100/100 | **Scope:** Validation 100/100 fixes: newsletter API never 500 on valid payload; data single-source (camp 3-day prices, overview fallbacks); UI (newsletter button black/white, logo sizes, PartnershipSection brand tokens). **Corrections:** (1) Form API 500 when rate limit or optional integration fails → rateLimit try/catch allow on failure; parseJsonBody return 400 on any parse error; void integration().catch(log); return 200 when validation passes. (2) parseJsonBody rethrow → catch all and return { ok: false, status: 400 }. (3) Hardcoded overview/camp prices in lib → move to pricing-supplemental.json programsOverviewFallbacks and year2026.json threeDayFullPrice/threeDayHalfPrice. **Patterns:** form-handler-never-500-on-valid-payload; overview-and-camp-prices-in-data; next-image-sizes-for-logos. **Anti-patterns:** optional-integration-causes-500; gray-in-brand-sections. **Quality bars:** formHandlerValidPayload200; nextImageSizesLogos; overviewFallbacksInData. |
...
| 2026-03-16 | `/compound:plan` Coach Hub private page | **Plan created:** `plans/coach-hub-private-page-implementation-plan.md`. Private coach-only Coach Hub: cookie-based auth (COACH_HUB_SECRET), middleware protection for /coach-hub, login at /coach-hub/login; migrate LBTA_Coach_Hub.html into Next.js (data in data/coach-hub/, client Hub with full parity). Use timing-safe secret compare, rate limit auth route, noindex for coach-hub; ConditionalLayout excludes coach-hub from main header/footer. |
| 2026-03-17 | `/compound:learn` after Coach Hub validation 100/100 | **Scope:** Coach Hub review fixes, full focus trap (Tab wrap), auth 400/401 verification script, validation summary 100/100. **Corrections:** Auth API 400/401 only verifiable when secret set → add verification script (scripts/verify-coach-hub-auth.mjs) and document how to run. **Patterns:** auth-contract-verification-script (script that asserts 400/401 with dev server + secret); modal-focus-trap-tab-wrap (Tab/Shift+Tab wrap at first/last focusable). **Anti-pattern:** protected-auth-api-without-runtime-verification. **Quality bar:** authContractVerificationScript (should). Updated corrections.jsonl, patterns.json, anti-patterns.json, quality-bars.json. |
| 2026-03-18 | `/compound:learn` after drop-in + private booking work | **Scope:** UTR drop-in pricing on schedules; private lesson flow from schedules and coach bios with coach pre-selection (plan: drop-in-and-private-booking-plan.md). **Learn doc:** .cursor/compound/learnings/2026-03-18-drop-in-private-booking-compound-learn.md. **Corrections:** Coach/schedules linking to /book without context → use /book?type=private&coach=slug and pre-select coach in modal; slug↔name from data/coaches.json via lib/coach-slug. **Patterns:** league-drop-in-in-data-and-ui (optional drop_in in division data + schema + row UI); contextual-private-booking-coach-slug (URL coach param → defaultCoach in modal). **Quality bar:** contextualBookingCoachPreSelect (should). Updated corrections.jsonl, patterns.json, quality-bars.json, COMPOUND_LEARN.md Latest. |
| 2026-03-19 | `/compound learn!` photos & layouts audit | **Scope:** Full audit (docs/PHOTOS-AND-LAYOUTS-AUDIT.md); all fixes applied (paths → legacy, hero alts, objectPosition, philosophy pillars, pixel a11y). **Doc:** [site-wide-image-404s-layout-consistency](../docs/solutions/ui-bugs/site-wide-image-404s-layout-consistency.md). **Links:** Audit and COMPOUND_LEARN point to solution; optional `scripts/verify-image-paths.mjs` for CI. |
| 2026-03-28 | `/compound:learn` after Lighthouse + proxy ship | **Scope:** Commit a4e51f7 — hero LCP (next/image priority poster, deferred video, fade on playing, reduced-motion poster-only), layout GA4+Pixel lazyOnload, removed global poster preload, FB dns-prefetch, middleware→proxy.ts. **Patterns:** hero-video-poster-lcp updated; nextjs16-proxy-coach-hub; analytics-scripts-lazyOnload-lcp. **Anti-patterns:** root-layout-preload-homepage-only-lcp; hero-poster-triple-fetch. **Quality bars:** homepageHeroLcpPosterImage; thirdPartyScriptLoadStrategy. **corrections.jsonl:** +2. **Doc:** `2026-03-28-lighthouse-proxy-compound-learn.md`. **Tradeoff:** lazyOnload analytics may delay first pageview vs afterInteractive — revisit if attribution requires earlier load. |
| 2026-03-19 | `/compound:full` agent-native audit fixes | **Scope:** Full agent-native architecture audit (8 principles, 37% overall score) + low-effort fixes (recommendations #3, #8, #9, #10). **Work:** ChatWidget suggested actions, /help page, API contracts doc, Racquet Rescue form fix, chat welcome alignment, accessibility fixes (close button, focus rings), performance optimization (sendMessage deps), brand token migration (lbta-* → brand-*). **Review:** Code WARN → fixed (a11y, brand tokens, performance); Security PASS; Performance PASS. **Deploy:** Commit 5aea252, push, vercel --prod. **Docs:** [AGENT-NATIVE-ARCHITECTURE-AUDIT.md](../docs/AGENT-NATIVE-ARCHITECTURE-AUDIT.md), [agent-native-architecture-audit.md](../docs/solutions/architecture/agent-native-architecture-audit.md), [api-contracts.md](../docs/api-contracts.md). **Learnings:** 4 corrections (ChatWidget close button a11y, sendMessage deps, legacy tokens, focus rings); 3 patterns (chat-suggested-actions, api-contracts-documentation, help-page-with-quick-links); 3 quality bars (chatWidgetCloseButtonA11y, chatWidgetSendMessageOptimized, brandTokensOnly); 5 anti-patterns (chat-widget-close-button-no-a11y, usecallback-messages-dependency, legacy-tokens-in-new-code, focus-ring-instead-of-focus-visible). |
